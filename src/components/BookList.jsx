...
import {DisplayMsg, Modal} from '../components';
...
class BookList extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    const {books, bookdata, isFetching} = nextProps;
    ...
    let books = this.state.books.map(function (book) {
      return (
          <Col xs={6} sm={4} md={3} key={book.best_book.id._}>
            <div onClick={that.showModal.bind(that, book.best_book.id._)}><Poster info id={book.best_book.id._} path={book.best_book.image_url} title={book.best_book.title}
                                                                                  responsive/>
            </div>
          </Col>
      );
